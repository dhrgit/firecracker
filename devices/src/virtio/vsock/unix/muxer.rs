...
/// `VsockMuxer` is the device-facing component of the Unix domain sockets vsock backend. I.e. by
/// implementing the `VsockBackend` trait, it abstracts away the gory details of translating between
/// AF_VSOCK and AF_UNIX, and presents a clean interface to the rest of the vsock device model.
///
/// The vsock muxer has two main roles:
/// 1. Vsock connection multiplexer:
///    It's the muxer's job to create, manage, and terminate `VsockConnection` objects. The muxer also
///    routes packets to their owning connections. It does so via a connection `HashMap`, keyed by
///    what is basically a (host_port, guest_port) tuple.
///    Vsock packet traffic needs to be inspected, in order to detect connection request packets
///    (leading to the creation of a new connection), and connection reset packets (leading to the
///    termination of an existing connection). All other packets, though, must belong to an existing
///    connection and, as such, the muxer simply forwards them.
/// 2. Event dispatcher
///    There are three event categories that the vsock backend is interested it:
///    1. A new host-initiated connection is ready to be accepted from the listening host Unix socket;
///    2. Data is available for reading from a newly-accepted host-initiated connection (i.e. the
///       host is ready to issue a vsock connection request, informing us of the destination port to
///       which it wants to connect);
///    3. Some event was triggered for a connected Unix socket, that belongs to a `VsockConnection`.
///    The muxer gets notified about all of these events, because, as a `VsockEpollListener` implementor,
///    it gets to register a nested epoll FD into the main VMM epolling loop. All other pollable FDs
///    are then registered under this nested epoll FD.
///    To route all these events to their handlers, the muxer uses another `HashMap` object, mapping
///    `RawFd`s to `EpollListener`s.
///
...
/// A unique identifier of a `VsockConnection` object. Connections are stored in a hash map, keyed by
/// a `ConnMapKey` object.
///
...
///
enum EpollListener {
    /// The listener is a `VsockConnection`, identified by `key`, and interested in the events in
    /// `evset`. Since `VsockConnection` implements `VsockEpollListener`, notifications will be
    /// forwarded to the listener via `VsockEpollListener::notify()`.
    Connection {
        key: ConnMapKey,
        evset: epoll::Events,
    },
    ...
    HostSock,
    /// A listener interested in reading host "connect <port>" commands from a freshly connected host
    /// socket.
...
pub struct VsockMuxer {
    ...
    /// The RX queue. Items in this queue are consumed by `VsockMuxer::recv_pkt()`, and produced
    ...
    /// The file system path of the host-side Unix socket. This is used to figure out the path to
    /// Unix sockets listening on specific ports. I.e. "<this path>_<port number>".
    host_sock_path: String,
    /// The nested epoll FD, used to register epoll listeners.
    epoll_fd: RawFd,
    /// A hash set used to keep track of used host-side (local) ports, in order to assign local ports
...
impl VsockChannel for VsockMuxer {
    ...
    fn recv_pkt(&mut self, pkt: &mut VsockPacket) -> VsockResult<()> {
        // We'll look for instructions on how to build the RX packet in the RX queue. If the queue
        // is empty, that doesn't necessarily mean we don't have any pending RX, since the queue might
        // be out-of-sync. If that's the case, we'll attempt to sync it first, and then try to pop
        // something out again.
        if !self.rxq.is_synced() && self.rxq.is_empty() {
            self.rxq.try_sync(&mut self.conn_map);
        }
        ...
        while let Some(rx) = self.rxq.pop() {
            let res = match rx {
                ...
                // We'll defer building the packet to this connection, that has something to say.
                MuxerRx::ConnRx(key) => {
            ...
            if res.is_ok() {
                // Inspect traffic, looking for RST packets, since that means we have to terminate
                // and remove this connection from the active connection pool.
                //
    ...
    ///
    /// This absorbs unexpected packets, handles RSTs (by dropping connections), and forwards all the
    /// rest to their owning `VsockConnection`.
    ///
    /// Returns:
    /// always `Ok(())` - the packet has been consumed, and its virtio TX buffers can be returned to
    /// the guest vsock driver.
    ///
    fn send_pkt(&mut self, pkt: &VsockPacket) -> VsockResult<()> {
        ...
        debug!("vsock: muxer.send[rxq.len={}]: {:?}", self.rxq.len(), *pkt.hdr);
        ...
        // We don't know how to handle packets addressed to other CIDs. We only handle the host part
        // of the guest - host communication here.
        ...
        if !self.conn_map.contains_key(&conn_key) {
            // This packet can't be routed to any active connection (based on its src and dst ports).
            // The only orphan / unroutable packets we know how to handle are connection requests.
        ...
        // However, if this is an RST, we have to forcefully terminate the connection, so there's
        // no point in forwarding it the packet.
        if pkt.hdr.op == uapi::VSOCK_OP_RST {
    ...
    /// Check if the muxer has any pending RX data, with which to fill a guest-provided RX buffer.
    ///
...
impl VsockEpollListener for VsockMuxer {
    /// Get the FD to be registered for polling upstream (in the main VMM epoll loop, in this case).
    ///
...
impl VsockMuxer {
    ...
    pub fn new(cid: u64, host_sock_path: String) -> Result<Self> {
        // Create the nested epoll FD. This FD will be added to the VMM `EpollContext`, at device
        // activation time.
        let epoll_fd = epoll::create(true).map_err(Error::IoError)?;

        // Open/bind/listen on the host Unix socket, so we can accept host-initiated connections.
    ...
    fn handle_event(&mut self, fd: RawFd, evset: epoll::Events) {
        debug!("vsock: muxer processing event: fd={}, evset={:?}", fd, evset);

        // We're (ab)using event notifications as a tick-timer to check for timed out connections,
        // since:
        // - this check is a pretty inexpensive operation; and
        // - we're under no pressure to respect any accurate timing for connection termination.
        self.sweep_killq();

        match self.listener_map.get_mut(&fd) {
            // This event needs to be forwarded to a `VsockConnection` that is listening for it.
            //
            Some(EpollListener::Connection { key, evset }) => {
                let key_copy = *key;
                let evset_copy = *evset;
                // The handling of this event will most probably mutate the state of the receiving
                // conection. We'll need to check for new pending RX, event set mutation, and all that,
                // so we're wrapping the event delivery inside those checks.
            ...
            Some(EpollListener::HostSock) => {
                if self.rxq.len() == unix_defs::MAX_CONNECTIONS {
                    // If we're already maxed-out on connections, we'll just accept and immediately
                    // discard this potentially new one.
                    info!("vsock: connection limit reached; refusing new host connection");
                ...
                self.host_sock
                    ...
                    .and_then(|stream| {
                        // Before forwarding this connection to a listening AF_VSOCK socket on the
                        // guest side, we need to know the destination port. We'll read that port
                        // from a "connect" command received on this socket, so the next step is to
                        // ask to be notified the moment we can read from it.
            ...
            // Data is ready to be read from a host-initiated connection. That would be the "connect"
            // command that we're expecting.
    ...
    /// I.e. as soon as we can also let our peer know we're dropping the connection, by sending it
    /// an RST packet.
    ///
    fn kill_connection(&mut self, key: ConnMapKey) {
        ...
        // This connection will now have an RST packet to yield, so we need to add it to the RX queue.
        // However, there's no point in doing that if it was already in the queue.
        if !had_rx {
            // We can safely ignore any error in adding a connection RX indication. Worst case scenario,
            // the RX queue will get desynchronized, but we'll handle that the next time we need to
            // yield an RX packet.
            self.rxq.push(MuxerRx::ConnRx(key)).unwrap_or_default();
        }
    }
    ...
    ///
    /// This will attempt to connect to a host-side Unix socket, expected to be listening at the file
    /// system path corresponing to the destination port. If successful, a new connection object will
    /// be created and added to the connection pool. On failure, a new RST packet will be scheduled
    ...
    ///
    /// This is used as shorthand for repetitive tasks that need to be performed after a connection
    /// object mutates. E.g.
        ...
        F: FnOnce(&mut VsockConnection),
    {
        if let Some(conn) = self.conn_map.get_mut(&key) {
            ...
            let was_shutting_down = conn.is_shutting_down();
            ...
            // If the connection wasn't previously scheduled for termination, add it to the kill queue.
            if !was_shutting_down && conn.is_shutting_down() {
                self.killq.push(key);
            }
            ...
            if new_evset.is_empty() {
                // If the connection no longer needs epoll notifications, remove its listener from
                // our list.
            ...
            if let Some(EpollListener::Connection { evset, .. }) = self.listener_map.get_mut(&fd) {
                if *evset != new_evset {
                    // If the set of events that the connection is interested in has changed, we need
                    // to update its epoll listener.
                    debug!(
                    ...
                    *evset = new_evset;
                    ...
                    .unwrap_or_else(|err| {
                        // This really shouldn't happen, like, ever. However, "famous last words" and
                        // all that, so let's just kill it with fire, and walk away.
                ...
                self.add_listener(fd, EpollListener::Connection {
                    key,
                    evset: new_evset,
                }).unwrap_or_else(|err| {
                    self.kill_connection(key);
    ...
    /// Check if any connections have timed out, and if so, schedule them for immediate termination.
    ///
    fn sweep_killq(&mut self) {
        // If the kill queue is out-of-sync, we'll need to synchronize it. No point in doing that
        // if there are still items that we could pop out, though.
        if !self.killq.is_synced() && self.killq.is_empty() {
            self.killq.try_sync(&mut self.conn_map);
        }
